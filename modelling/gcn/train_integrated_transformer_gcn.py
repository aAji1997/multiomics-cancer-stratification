...
def run_integrated_training(args):
    ...
    run_name = f"integ-transformer-gcn-{args.cancer_type}-{run_timestamp}"
    ...
    # ADDED: Freeze gene embeddings to use them as fixed context
    ...
    if args.train_epochs <= 0:
        ...
        if gene_masks and not args.ignore_gene_masks:
            ...
            def masked_omics_mse_loss(pred, target):
                ...
                for i, modality in enumerate(sorted(omics_input_dims.keys())):
            ...
            try:
                ...
                if args.use_mixed_precision and device.type == 'cuda':
                    with torch.cuda.amp.autocast():
                        ...
                        # MODIFIED: Calculate graph loss only for logging, not for gradient (as gene embeddings are frozen)
                        loss_graph = torch.tensor(0.0, device=device)
                        if args.graph_loss_weight > 0 and graph_loss_fn is not None:
                            with torch.no_grad():  # Calculate without tracking gradients (for logging only)
                        ...
                        # --- Combine Losses ---
                        # MODIFIED: Remove graph_loss from total_loss calculation since gene embeddings are frozen
                        total_loss = (loss_contrastive +
                                      # args.graph_loss_weight * loss_graph + # Removed for Strategy 3
...
if __name__ == "__main__":
    ...
    if args.output_dir:
        ...
        run_folder_name = f"{args.cancer_type}_integrated_T{args.transformer_layers}L_G{args.gcn_layers}L_{args.gcn_conv_type}_{decoder_info}_E{args.train_epochs}_{time_stamp}"
